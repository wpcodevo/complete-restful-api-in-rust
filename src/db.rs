...
impl UserExt for DBClient {
    async fn get_user(
        ...
        if let Some(user_id) = user_id {
            user = sqlx::query_as!(User, "SELECT * FROM users WHERE id = $1", user_id)
                .fetch_optional(&self.pool)
                .await?;
        } else if let Some(name) = name {
            user = sqlx::query_as!(User, "SELECT * FROM users WHERE name = $1", name)
                .fetch_optional(&self.pool)
                .await?;
        } else if let Some(email) = email {
            user = sqlx::query_as!(User, "SELECT * FROM users WHERE email = $1", email)
                .fetch_optional(&self.pool)
                .await?;
        }
    ...
    async fn get_users(
        ...
        let users = sqlx::query_as!(
            User,
            r#"SELECT * FROM users WHERE (name LIKE $1) OR (email LIKE $2)
            LIMIT $3 OFFSET $4"#,
            query,
            query,
            page_size as i64,
